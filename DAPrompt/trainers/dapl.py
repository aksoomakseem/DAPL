...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        #tokenized_prompts[dm*cls+cls, 77, 512]
        return x
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.DAPL.N_CTX #16
        ...
        n_dm = len(cfg.DATASET.SOURCE_DOMAINS) + len(
            cfg.DATASET.TARGET_DOMAINS)  # number of domains,1+1=2
        n_dmx = cfg.TRAINER.DAPL.N_DMX  # number of domain context,16
        n = n_dmx + n_ctx #16+16=32
        self.n_dm = n_dm
        self.n_dmx = n_dmx
        ...
        if cfg.TRAINER.DAPL.CSC:
            ...
            ctx_vectors = torch.empty(n_cls, n_ctx, ctx_dim, dtype=dtype) #cls*16*512
        else:
        ...
        domain_vectors = torch.empty(n_dm, n_dmx, ctx_dim, dtype=dtype) #2*16*512
        nn.init.normal_(domain_vectors, std=0.02)
        self.domain_vectors = nn.Parameter(domain_vectors) #将域特定上下文向量注册为可训练的模型参数

        print(f'Initial context: "{prompt_prefix}"')
        print(f"Number of context words (tokens): {n_ctx}")
        print(f"Number of domain context words (tokens): {n_dmx}")

        self.ctx = nn.Parameter(ctx_vectors)  #将域无关上下文向量注册为可训练的模型参数
        ...
        ]  #"X X X"         " " "blue cat"  " "   ", a REAL image"  "."

        tokenized_prompts = torch.cat([clip.tokenize(p) for p in prompts]) #[dm*cls, 77]
        naive_tokenized_prompts = torch.cat([clip.tokenize(p) for p in naive_prompts]) #[cls, 77]

        with torch.no_grad():
            embedding = clip_model.token_embedding(tokenized_prompts).type(dtype) #[dm*cls, 77, 512]
            naive_embedding = clip_model.token_embedding(naive_tokenized_prompts).type(dtype) #[cls, 77, 512]

        # These token vectors will be saved when in save_model(),
        # but they should be ignored in load_model() as we want to use
        # those computed using the current class names
        tokenized_prompts = torch.cat([tokenized_prompts, naive_tokenized_prompts]) #[dm*cls+cls, 77, 512] 
        self.register_buffer("token_prefix", embedding[:, :1    , :])  # SOS [dm*cls, 1, 512]
        self.register_buffer("token_suffix", embedding[:, 1 + n:, :])  # CLS, EOS [dm*cls, 77-n-1, 512]
    ...
    @autocast()
    def forward(self):
        ctx = self.ctx #[16,512]or[cls,16,512]
        ctx_dim = ctx.size(-1)
        dmx = self.domain_vectors  #[dm,16,512]
        if ctx.dim() == 2: #通用上下文
            ctx = ctx.unsqueeze(0).expand(self.n_dm, -1, -1)  #[16,512]->[dm,16,512]
            if not self.csc:
                ctx = ctx.unsqueeze(1).expand(-1, self.n_cls, -1, -1)  #[dm,16,512]->[dm,cls,16,512]
        else: #类特定上下文
            ctx = ctx.unsqueeze(0).expand(self.n_dm, -1, -1, -1)  # [cls,16,512]->[dm,cls,16,512]
                                               # 将类特定上下文向量扩展为所有域共享的形状

        dmx = dmx.unsqueeze(1).expand(-1, self.n_cls, -1, -1)  #[dm,16,512]->[dm,cls,16,512]
                                               # 将域特定上下文向量扩展为所有类别共享的形状
        ctxdmx = torch.cat([ctx, dmx],
                           dim=2).reshape(self.n_cls * self.n_dm,
                                          self.n_ctx + self.n_dmx, ctx_dim) #[dm*cls, 16+16(n), 512]

        prefix = self.token_prefix #[dm*cls, 1, 512]
        suffix = self.token_suffix #[dm*cls, 77-n-1, 512]

        # naive
        neb = self.naive_embedding #[cls, 77, 512]

        prompts = torch.cat(
            [
                prefix,  # (dm*cls, 1, dim)
                ctxdmx,  # (dm*cls, n, dim)
                suffix,  # (dm*cls, 77-n-1, dim)
            ],
            dim=1,
        ) #[dm*cls, 77, 512]
        prompts = torch.cat([prompts, neb], dim=0) #[dm*cls+cls, 77, 512]
